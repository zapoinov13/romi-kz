-- Фикс прав в reconcile_cdi_for_project: разрешаем вызов из SQL Editor (service_role).
--
-- Проблема: функция объявлена с SECURITY DEFINER + проверкой auth.uid()/has_role,
-- но при вызове из Supabase Dashboard SQL Editor сессия идёт под service_role БЕЗ JWT.
-- auth.uid() возвращает NULL → has_role(NULL, 'admin') = false → forbidden.
--
-- Решение: пропускать проверку, если auth.uid() IS NULL — это значит вызов идёт от
-- service_role (cron / SQL Editor / Supabase Functions admin-call). У service_role
-- и так все права, отдельная проверка избыточна.
...
BEGIN
  -- Разрешаем вызов: service_role (auth.uid IS NULL), admin, или член проекта.
