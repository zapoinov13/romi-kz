// Шлёт тестовое сообщение в чат проекта.
...
Deno.serve(async (req) => {
  ...
  const { data: bot, error: loadErr } = await admin
    .from("project_telegram_bots")
    .select("bot_token, chat_id, bot_username")
  ...
  try {
    const res = await fetch(`https://api.telegram.org/bot${bot.bot_token}/sendMessage`, {
      ...
      body: JSON.stringify({
        chat_id: bot.chat_id,
        text: "✅ Бот подключён к проекту. Готовые креативы будут приходить сюда.",
      }),
    ...
    if (res.ok && j.ok) {
      ...
      errorMessage = j.description ?? `HTTP ${res.status}`;
    }
  } catch (e) {
