-- Lovable SQL Editor: Аналитика продаж
...
CREATE TRIGGER trg_sync_sales_analytics_from_lead
  AFTER INSERT OR UPDATE OF name, phone, meta_ad_id, utm, campaign, source, channel, project_id, cabinet_id
  ON public.leads
  FOR EACH ROW
...
CROSS JOIN (
  VALUES
    ('Настройка рекламы', 500::numeric, 1),
    ('AI Агент', 1200::numeric, 2),
    ('CRM', 2000::numeric, 3)
...
-- RLS (пересоздаём политики — скрипт можно запускать повторно)
...
DROP POLICY IF EXISTS sales_service_catalog_select ON public.sales_service_catalog;
DROP POLICY IF EXISTS sales_service_catalog_insert ON public.sales_service_catalog;
DROP POLICY IF EXISTS sales_service_catalog_update ON public.sales_service_catalog;
DROP POLICY IF EXISTS sales_service_catalog_delete ON public.sales_service_catalog;
DROP POLICY IF EXISTS sales_analytics_leads_select ON public.sales_analytics_leads;
DROP POLICY IF EXISTS sales_analytics_leads_insert ON public.sales_analytics_leads;
DROP POLICY IF EXISTS sales_analytics_leads_update ON public.sales_analytics_leads;
DROP POLICY IF EXISTS sales_analytics_leads_delete ON public.sales_analytics_leads;
...
-- Realtime (игнорируем, если таблица уже в публикации)
DO $$
BEGIN
